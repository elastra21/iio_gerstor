<template>
  <el-row class="vinculacion">
    <div class="header-section">
      <h2>Incidencia Sociál</h2>
    </div>
      ...
      <el-card class="form">
        ...
        <el-form>
          <el-form-item label="Nombre">
            <el-input v-model="vinculacion.name" :showError="false"></el-input>
          </el-form-item>

          <el-form-item label="Título">
            <el-input v-model="vinculacion.subtitle" :showError="false" />
          </el-form-item>

          <el-form-item label="Link">
            <el-input v-model="vinculacion.link" :showError="false" />
          </el-form-item>

          <el-form-item label="Descripción">
            <el-input
              type="textarea"
              v-model="vinculacion.description"
              :showError="false"
            />
          </el-form-item>
          <el-form-item>
    ...
    <el-col :span="12">
      <el-card class="box-card list">
        ...
        <el-table :data="data.vinculacion.vinculacion" style="width: 100%">
          <el-table-column prop="subtitle" label="Título" />
...
export default {
  ...
  methods: {
    ...
    getName() {
      ...
      if (editingItem > -1) {
        const { subtitle } = data.vinculacion.vinculacion[editingItem];
        return subtitle;
      } else return "";
    },
    confirmDelete(index) {
    ...
    validateFields() {
      const { name, subtitle, description, link } = this.vinculacion;
      return (
        name.length > 0 &&
    ...
    saveData() {
      const { vinculacion, image } = this;
      const formData = new FormData();
      formData.append("operation", "add");
      formData.append("section", "vinculacion");
      formData.append("field", "vinculacion");
      const json = { ...vinculacion };
      if (image !== null) {
        formData.append("image", image);
        json.image = this.normalizeString(`${vinculacion.name}.jpg`);
      } else json.image = "";
      formData.append("data", JSON.stringify(json));
      this.submitRequest(formData);
    },
    editData() {
      const { editingItem, image } = this;
      const { vinculacion } = this;
      const formData = new FormData();
      ...
      if (image !== null) {
        ...
        const vinculacion_old = this.data.vinculacion.vinculacion[editingItem];
        json.image =
          vinculacion_old.image.length > 0 ? vinculacion_old.image : "";
      }
      formData.append("data", JSON.stringify(json));
      this.submitRequest(formData);
    },
    ...
    handleEdit(index, slide) {
      this.editing = true;
      this.vinculacion = { ...slide };
      delete this.vinculacion.image;
      this.editingItem = index;
      slide.image.length > 0
        ? (this.editImage =
            $(location).attr("origin") + "/archivos/vinculacion/" + slide.image + "?" + new Date().getTime())
        : (this.editImage = null);
    }
  },
  beforeMount: function() {
    ...
    const news = [...this.data.vinculacion.vinculacion];
    this.model.vinculacion = news;
